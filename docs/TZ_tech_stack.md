...
- **Точное имя npm-пакета MAX Mini Apps SDK по-прежнему не подтверждено**
  (открытый пункт п. 8.8). До получения официальной документации весь доступ
  к платформе изолируется в один модуль-адаптер (`initData`, нативная кнопка
  «назад», haptics, тема), чтобы подключение реального SDK не расползлось по
  коду. **Ссылку на документацию заказчик обещал передать — её нужно
  запросить перед началом работ по интеграции.**
