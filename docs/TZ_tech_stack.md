...
- **Реальные с самого начала:** hh.ru (публичный API без ключа, кэш 6 часов,
  фолбэк на кэш при недоступности — У14), картографический API для блока 7
...
официальной организацией `max-messenger` на GitHub и реестром npm.
...
- **Транспорт уведомлений** (`MaxBotTransport`, раздел 9.1) строится на
  официальном клиенте Bot API. Для Core API/Worker на Python это
  `max-botapi-python`; отдельный Node-процесс ради `@maxhub/max-bot-api`
  не заводим.
